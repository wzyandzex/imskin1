...
| **密钥安全**：api key 由调用方注入（环境变量/配置）；前端不直连（CORS/key 泄露），需经后端代理 | LiteLLM 环境变量/YAML、Ollama「key 必填但被忽略」 | ✅ `registryFromEnv` 读环境变量；前端走 REST 代理 |
...
- `llm-core`：**9 测试全绿**（registry 解析/别名、LLM 成功、无 provider 降级、非法 JSON 降级、结构不合法降级、网络错降级、strict→json_object 回退、markdown 围栏提取、401 不重试）——全部用 `node:test` 的 `mock.method(fetch)` 离线验证。
- `api`：**5 测试全绿**（同步生成+导出、缺参失败、REST 全链路、Bearer 认证 401、未配 key 本地放行）。
- CLI 实跑验证：`generate` 产出版本 + 导出四个真实包（.ssf/.bps/.bds 字节非空）。

## 4. 诚实边界

- LLM 结构化输出的**真实质量**取决于所接模型；本层只保证"接入 + 结构化 + 降级"，不保证某模型的审美判断。
- REST server 是**本地/自托管**形态；要做多租户云端服务需再加 配额/速率限制/对象存储（NFR-SCALE/NFR-COST），本期范围外。
