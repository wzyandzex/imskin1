...
describe("FR-PREVIEW-7 多机型 DPI 档位预览", () => {
  test("手机形态提供 标清/高清/超清 三档切换", async () => {
    ...
    expect(screen.getByRole("button", { name: "标清" })).toBeTruthy();
    expect(screen.getByRole("button", { name: "高清" })).toBeTruthy();
    expect(screen.getByRole("button", { name: "超清" })).toBeTruthy();
  });
  ...
  test("切换档位 → 外壳标注当前模拟的 DPI（不混淆，AC2）", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "超清" }));
